...
void searchByTopic(TreeNode* root, const string& topic, vector<string>& bodies) {
    if (root->tagName == "post") {
        ...
        // Traverse children of the "post" node
        for (TreeNode* child : root->children) {
            if (child->tagName == "topics") {
                // Check if the desired topic exists in the "topics" node
                for (TreeNode* topicNode : child->children) {
                    if (topicNode->tagName == "topic" && toLower(topicNode->value) == toLower(topic)) {
                        topicFound = true;
                        break;
                    }
                }
                ...
                bodyContent = child->value;
            }
        }

        // If the topic is found and body content exists, add the body to the result
        if (topicFound && !bodyContent.empty()) {
            bodies.push_back(bodyContent);
        }
    }

    // Recursively process child nodes (to ensure all posts are checked)
    for (TreeNode* child : root->children) {
        searchByTopic(child, topic, bodies);
    }
}
...
void searchByWord(TreeNode* root, const string& word, vector<string>& bodies) {
    ...
    for (TreeNode* child : root->children) {
        searchByWord(child, word, bodies);
    }
}
