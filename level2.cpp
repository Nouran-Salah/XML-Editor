...
void searchByTopic(TreeNode* root, const string& topic, int& count) {
        ...
        if (topicFound) {
            for (TreeNode* child : root->children) {
                if (child->tagName == "body") {
                    cout << "Post (Body) " << ++count << " : " << child->value << endl;
                }
            }
        }
    }
    ...
    for (TreeNode* child : root->children) {
        searchByTopic(child, topic, count);
    }
}

void searchByTopic(TreeNode* root, const string& topic) {
    int count = 0;
    searchByTopic(root, topic, count);
    if (count == 0) {
        cout << "The topic '" << topic << "' was not found in any post." << endl;
    }
}


// Updated searchByWord function
